...
## ISM couplings (model revision 6) — 2026-09-22

Four particle-mesh couplings now change gas after every leapfrog step, on top of the revision-5 cooling, pressure, ram drag, and blastwave delay. This is still **not SPH** and not a chemical network. Isolated `n_galaxies=1` collisionless ICs are unchanged (same DF; generator stamped 6). Historical `validation_r5.json` and `api_validation_r5.json` were not rewritten. `validate_million.py` now asserts revision 6 and was **not** run.

**What was added.**
- Snowplow: `sn_momentum` (default 0.4) shares a momentum-conserving kick between neighbouring gas and the remnant. Speed is capped at 1.2 code units (~144 km/s) per event. The stored diagnostic is the scalar impulse delivered to the gas, so a symmetric shell is not reported as zero.
- Cloud drag: `cloud_dissipation` (default 1) damps peculiar motion inside a density cell. Bulk velocity is unchanged. Lost kinetic energy heats the gas.
- Metal mixing: `metal_diffusion` (default 0.6) exchanges metal mass across cell faces. Total metal mass is conserved. Intra-cell differences stay on the particles.
- Local FUV: `fuv_heating` (default 1). Stars ≥8 M☉ and younger than 30 Myr brighten their cell and the six neighbours. Dense gas is shielded. With no young stars the knob does not change the heating law.
- Electron fraction `x_e` (`noneq_ionization`, default on) relaxes toward coronal plus photo equilibrium. Metal-line cooling is scaled by how close `x` is to that equilibrium. Pressure still uses `u`, so ionization changes forces only by changing the thermal energy.

**Knobs.** Compute group Interstellar medium. Draft key `orbital-lab-draft-v4`. Estimator ×1.18 and engine-RAM factor 1.45 when ISM and lifecycle are on are **predictions**, not new measured references. Baryon checkpoints store `x` plus the snowplow and cloud-heat scalars. Resuming an older checkpoint that lacks `x` fills a warm-gas equilibrium fraction and then integrates with the live operators.

**Measured physics.** `PYTHONPATH="$PWD/work/openmp:$PWD/outputs/observatory" work/venv/bin/python outputs/observatory/tests/validate_physics.py` → `validation_r6.json` (63 s, pip REBOUND 5.1.1, no OpenMP tree). Isolated vs `n_galaxies=1` max |Δstate| = 0; vs `origin/main` revision-3 `physics.py` max |Δstate| = 0. Lifecycle-off 2048 energy 4.40e-5 / 5.69e-6. Lifecycle-off ISM-off speed-40 500 steps: mass drift 0, **307 births / 60 deaths / 5 SN**. Cooling peak 2.47×10^5 K. Delayed parcels stay at 5.00×10^5 K; the undelayed parcels in that dense cell fall to **2257 K** (shielded floor is 1000 K; the revision-5 run of the same setup landed near 7954 K on the 8000 K floor). Ram test: momentum drift 0, v_rel 2.4 → 1.02, internal-energy ratio 77.7 (revision 5 recorded 87.9). Hot gas still rejected for star formation. ISM+lifecycle 80 steps: mass drift 0, mean T **5197 K** (revision 5 was 9742 K), births 293, hot mass 0. Two-galaxy ISM 40 steps: mass drift 0, finite, separation 18.55, hot mass **0** at the end (revision 5 ended at 0.007). Resume max |Δstate| including `x` = 0. `peek` still leaves the SFR window alone.

**Measured levels** (same file). One 25 M☉ death, gravity off, heat coupling 0: snowplow 0 / 0.5 / 1.5 gives radial gas speed **0 / 9.91 / 29.7 km/s**, momentum residual ~10⁻³⁴, scalar impulse 0 / 0.00735 / 0.0221. A forced huge coupling stops at speed 1.20. Cloud drag over 40 steps, one cell, cooling off: peculiar rms **0.0761 / 0.0541 / 0.0341** at coefficients 0 / 1 / 3, specific energy 0.00884 / 0.0103 / 0.0112, cloud heat 0 / 3.44e-5 / 5.56e-5, momentum drift ≤ 3e-19. Metal mixing over 40 steps, two adjacent cells: contrast **0.0390 / 0.0319 / 0.0177** at 0 / 0.5 / 2, metal-mass drift ~10⁻¹⁹. FUV heat with no stars is 2.00×10⁻²² erg s⁻¹ per H at both knob 0 and knob 5. A 200 M☉ young-star light raises that to 3.66×10⁻²²; at n_H = 30 cm⁻³ the shield is 0.00111 and the heat is 4.07×10⁻²⁵. Twelve-step temperatures: n_H = 0.05 cm⁻³ sits on the **3×10⁷ K** ceiling for every FUV setting; n_H = 0.5 separates cleanly (**1.995×10⁴ / 1.995×10⁴ / 3.981×10⁴ / 1.000×10⁵ K** for no star, star with FUV 0, FUV 1, FUV 4); n_H = 5 stays at 5012 K until FUV 4 lifts it to 10⁴ K; n_H = 20 stays at **1000 K** even at FUV 4. Ionization over one 0.49 Myr step from x = 0.02 at 10⁴ K: lag **0.616 / 0.615 / 0.495 / 4.6×10⁻⁵** at n_H = 10⁻⁶ / 10⁻⁴ / 10⁻² / 1 cm⁻³. At 1 cm⁻³, x = 0.215 matches equilibrium. Between 10⁻⁶ and 10⁻⁴ the lag barely moves because photoionization, not density, sets the rate. A 1024-particle galaxy, 30 steps, speed 40, three knob levels: mass drift 0, mean T 4013 K, x lag 0, 106 births, 2 SN, and **0 face-adjacent gas cells**, so metal spread stays 0.125 at every mixing rate. Cloud heat is 0 / 7.35×10⁻⁵ / 2.08×10⁻⁴. Snowplow impulse is 0 at coupling 0 and **3.31×10⁻⁴ at both 0.4 and 1.5**, because the 144 km/s cap already binds on these superparticle ejecta.

**Measured API.** `PYTHONPATH="$PWD/work/openmp:$PWD/outputs/observatory" work/venv/bin/python outputs/observatory/tests/validate_api.py` → `api_validation_r6.json` (39 s, isolated port 8767). Revision 6; defaults `sn_momentum` 0.4, `noneq_ionization` true; baryon npz includes `x`; pause/restart, 201 frames, first frame identical; history gas T has **139** distinct rounded samples and a final mean electron fraction 0.0528; summary 6858 B; over-budget text “Estimated **662 h** exceeds the 120-hour compute cap” (the ×1.18 factor; revision 5 said 629 h); planets energy 2.78×10⁻¹⁶. That API galaxy ended at 1198 births, 214 deaths, 24 SN (revision 5 was 1198 / 219 / 25). Historical validation mtimes are unchanged (`validation.json` / r3 / r4 at 2026-09-21 23:59:56 UTC; r5 and api r5 at 2026-09-22 14:02:54 UTC).

**Browser.** Tmux `observatory-8766`, `OBSERVATORY_DATA=/workspace/work/observatory-data`, `daemon=false`, `worker_pid=null`. Disposable job `48f7e58d1129` **complete** (10,000 particles, 1 galaxy, duration 1, lifecycle speed 40, ISM on, 51 frames, about 11 s wall): T 17,542 K, x_e 0.620, lag 1.5×10⁻⁶, metal spread 0.148, cloud heat 9.70×10⁻⁴, snowplow impulse 9.16×10⁻⁴, hot mass 7.70×10⁻⁴, 435 births / 93 deaths / 12 SN. That is one short run, not a stability proof. Compute shows the five new controls at 0.40, 1, 0.60, 1, and ionization on. Lifecycle off dims every ISM row, including the master checkbox, with “not used unless stellar lifecycle is on”. ISM off leaves the master checkbox usable and dims the rest. Narrow ~700 px: the controls stack and stay reachable. Compute network filter `frames` was 0 of 72 requests. Observe inspector reads **17,542 K · xₑ 0.62**. No console errors. No 1M job was started. Ctrl+C is a graceful shutdown.

**Outstanding / honest limits.** Superparticles; no SPH kernel or Riemann solver; SN coupling and the snowplow speed are laboratory scales; blast delay is 15 simulation Myr, so `lifecycle_speed=40` can stack blastwaves; the unshielded floor is 8×10³ K and the shielded floor is 10³ K; below ~0.2 cm⁻³ the laboratory heating rate runs into the 3×10⁷ K ceiling, so FUV levels only separate temperature near ~0.5 cm⁻³; at N=1024 the gas cells in this disk are not face-adjacent, so mixing does nothing until particles share a face; galaxy-density gas reaches ionization equilibrium inside one leapfrog step, and the lag is a diffuse-gas effect. Energy change with the ISM on is not an error bound. Mean T in the 30- and 80-step runs is not a calibrated two-phase ISM and does not prove long-term stability.
