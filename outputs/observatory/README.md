...
- **Galaxy (model revision 6):** 1–5 live N-body galaxies on one leapfrog tree. Galaxy A keeps the full structure/lifecycle/ISM knobs; galaxies 2–5 are compact clones (mass, size, placement, velocity, spin). Isolated `n_galaxies=1` still bit-matches the revision-3 disk+halo (same DF; generator stamped 6). Default new Compute job is **2 galaxies**. Optional central black hole, stellar lifecycle, and a laboratory two-phase ISM (grid pressure, ram drag, snowplow momentum, cloud drag, metal mixing, local FUV with shielding, a carried electron fraction, CIE-like cooling, blastwave-delayed SN heat). All particles contribute gravity. 10,000–1,000,000 particles shared across the live galaxies; 1–14 CPU threads; span limited by a 120-hour compute cap. This is not a calibrated MW–M31 run, not SPH, and not a prescribed merger movie.
...
When **two-phase ISM** is on, gas parcels carry specific internal energy `u`, metallicity `Z`, and electron fraction `x_e`. After every leapfrog step they feel a momentum-conserving grid pressure force, ram drag, and — when a massive star dies — a momentum-conserving snowplow on neighbouring gas. Peculiar motion inside a cell is dissipated into heat without changing the cell’s bulk velocity. Metals exchange across cell faces; the metal mass is conserved. CIE-like cooling Λ(T, Z) has a metal-line peak near 10^5.4 K. Young massive stars add local FUV, and heating and photoionization are shielded in dense gas. `x_e` relaxes toward coronal plus photo equilibrium; at galaxy densities that relaxation usually finishes inside one 0.49 Myr leapfrog step, so the lag shows up in diffuse gas. Stinson-like delayed cooling lasts ~15 Myr after a supernova dump. Star formation is allowed only in cold (T < 3.5×10^4 K), dense (n_H above `n_sf`), non-expanding gas. This is **not SPH**: there is no kernel, no Riemann solver, no resolved Jeans mass, and no chemical network. Superparticle SN coupling (`sn_feedback`, `sn_momentum`) is a laboratory parameter because one slot is 10^4–10^6 suns, not 10^51 erg per resolved star. Energy change is not a numerical-error bound when the lifecycle or ISM is on. Turn the ISM off to recover the revision-4 collisionless bookkeeping.
...
- **Revision 6 evidence** (`validation_r6.json`, `api_validation_r6.json`): the same collisionless checks still pass, including 307/60/5 with the ISM off. Snowplow radial speed rises from 0 to 9.91 to 29.7 km/s at couplings 0, 0.5, and 1.5, with momentum conserved and a 144 km/s cap. Cloud drag lowers peculiar speed and raises internal energy. Face mixing lowers metal contrast and keeps the metal mass. Local FUV does nothing with no young stars, raises temperature near 0.5 cm⁻³, and is shut off at 20 cm⁻³ (1000 K). Electron fraction lags at 10⁻⁶–10⁻² cm⁻³ and matches equilibrium at 1 cm⁻³. A 1024-particle, 30-step galaxy conserves mass at low, default, and high knobs; its gas cells are not face-adjacent, and the snowplow cap makes couplings 0.4 and 1.5 deposit the same impulse. These short runs are not a stability proof. The ×1.18 wall-time factor is a prediction.
...
- `ism.py`: laboratory particle-mesh ISM (cooling, pressure, ram, snowplow, cloud drag, metal mixing, local FUV, non-equilibrium ionization).
...
- `tests/validate_physics.py`, `tests/validate_api.py`, `tests/validate_queue.py`: numerical, recovery, and queue checks (write `validation_r6.json` / `api_validation_r6.json` / `queue_validation.json`; never overwrite historical `validation.json` / `validation_r3.json` / `validation_r4.json` / `validation_r5.json`).
