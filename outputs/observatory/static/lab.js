...
else if(s.kind==='choice'){input=document.createElement('input');input.type='range';input.min=0;input.max=s.stops.length-1;input.step=1;input.value=Math.max(0,s.stops.indexOf(Number(v)));const ticks=document.createElement('div');ticks.className='ticks';s.stops.forEach((st,i)=>{const t=document.createElement('span');t.textContent=(s.labels&&s.labels[i])||fmt(st);ticks.append(t)});row.append(ticks)}
...
out.textContent=readout(s,v);row.prepend(label);row.append(input,out,unit);rows[s.id]={input,out,slider:s,row,hint:label.querySelector('small')};box.append(row)}
$('sliders').append(box)}updateEstimate()}
function markUnused(){if(mode!=='galaxy')return;const G=Number(values.galaxy.n_galaxies||1);
for(const id in rows){const {out,slider:s,row}=rows[id];if(!s.galaxy)continue;const unused=G<s.galaxy;row.classList.toggle('unused',unused);out.textContent=unused?`not used unless galaxy count ≥ ${s.galaxy}`:readout(s,values.galaxy[s.id])}}
