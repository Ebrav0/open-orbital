...
function readout(s,v){if(s.kind==='bool')return v?'on':'off';let text=s.kind==='choice'||s.kind==='number'||Number.isInteger(s.step)?fmt(v):Number(v).toLocaleString('en-US',{maximumFractionDigits:4});if(s.physical){const p=s.physical(Number(v));if(p)text+=` · ${p}`}return text}
...
for(const s of list.filter(x=>x.group===g)){const row=document.createElement('div');row.className='slider-row';const label=document.createElement('label');label.htmlFor=`k-${s.id}`;label.textContent=s.label;if(s.hint){const hint=document.createElement('small');hint.textContent=s.hint;label.append(hint)}
...
else if(s.kind==='choice'){input=document.createElement('input');input.type='range';input.min=0;input.max=s.stops.length-1;input.step=1;input.value=Math.max(0,s.stops.indexOf(Number(v)));const ticks=document.createElement('div');ticks.className='ticks';s.stops.forEach(st=>{const t=document.createElement('span');t.textContent=fmt(st);ticks.append(t)});row.append(ticks)}
else{input=document.createElement('input');input.type='range';input.min=s.min;input.max=s.max;input.step=s.step;input.value=v}
input.id=`k-${s.id}`;input.setAttribute('aria-label',s.label);const unit=document.createElement('span');unit.className='unit';unit.textContent=s.unit||'';
input.addEventListener('input',()=>{let val;if(s.kind==='bool')val=input.checked;else if(s.kind==='choice')val=s.stops[Number(input.value)];else if(s.kind==='number'){val=Math.min(s.max,Math.max(s.min,Math.round(Number(input.value)||0)))}else val=Number(input.value);values[mode][s.id]=val;out.textContent=readout(s,val);updateEstimate();scheduleSave()});
out.textContent=readout(s,v);row.prepend(label);row.append(input,out,unit);rows[s.id]={input,out,slider:s,hint:label.querySelector('small')};box.append(row)}
$('sliders').append(box)}updateEstimate()}
function syncInputs(){for(const id in rows){const {input,out,slider:s}=rows[id],v=values[mode][id];if(s.kind==='bool')input.checked=Boolean(v);else if(s.kind==='choice')input.value=Math.max(0,s.stops.indexOf(Number(v)));else input.value=v;out.textContent=readout(s,v)}updateEstimate()}
function applyDurationCap(cfg){if(!rows.duration)return;const cap=durationCap(cfg);const s=rows.duration.slider;if(s.kind==='choice')return;rows.duration.input.max=cap;if(Number(values[mode].duration)>cap){values[mode].duration=cap;rows.duration.input.value=cap;rows.duration.out.textContent=readout(s,cap);scheduleSave()}if(mode==='galaxy'&&rows.duration.hint)rows.duration.hint.textContent=`Live max ${fmt(cap)} model units (${Math.round(cap*MYR_PER_TIME).toLocaleString('en-US')} Myr) from the 120 h wall-time estimate.`}
...
$('estimate-time').textContent=`${fmtHours(hours)} estimated wall`;
...
if(s.phase==='pausing')return 'Integrator reached a pause point. Writing a checkpoint now — this can take several seconds at 200,000 particles.';
...
function renderStatus(){const j=target();if(!j){$('status-heading').textContent='Nothing selected';$('status-sub').textContent='Start a computation or pick a saved experiment.';$('phase').textContent='Idle';$('phase').dataset.phase='idle';$('progress-bar').style.width='0%';renderStopButton(null);pauseEta=null;renderPauseEta();$('diagnostics').replaceChildren();return}
...
armPauseEta(s);
$('status-heading').textContent=m.title||(c.mode==='galaxy'?'Building galaxy initial conditions':'Building planetary system');$('status-sub').textContent=`${c.mode==='galaxy'?`${fmt(c.n)} particles · ${c.threads} threads`:`${m.n||c.n} bodies`} · span ${c.mode==='galaxy'?`${Math.round(c.duration*MYR_PER_TIME)} Myr`:`${c.duration} yr`}${c.notes?` · ${c.notes}`:''}`;
...
renderStopButton(j);renderPauseEta();
...
function renderRuns(){const list=jobs.filter(j=>j.config.mode===mode);$('run-count').textContent=list.length;const box=$('run-list');box.replaceChildren();const t=target();
for(const j of list){const card=document.createElement('div');card.className='run lab-run'+(j.id===t?.id?' active':'');const pick=document.createElement('button');pick.className='run-pick';const name=document.createElement('span');name.textContent=j.config.mode==='galaxy'?`${fmt(j.config.n)} particles · ${j.config.threads} threads · ${Math.round(j.config.duration*MYR_PER_TIME)} Myr${j.meta?.lifecycle_enabled?' · lifecycle':''}`:`${j.meta?.title||'Solar System'} · ${j.config.duration} yr`;const small=document.createElement('small');small.textContent=`${phaseLabel(j.status)} · rev ${j.meta?.model_revision??'?'} · ${new Date(j.config.created*1000).toLocaleString([],{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})} · ${j.id}${j.config.notes?` · ${j.config.notes.slice(0,60)}`:''}`;pick.append(name,small);pick.title='Load these settings as a draft and show this run\'s status';
pick.onclick=()=>{selectedId=j.id;values[mode]=fromConfig(mode,j.config);syncInputs();scheduleSave();renderStatus();renderRuns();toast(`Loaded settings from ${j.id} as a draft. Press Start computation to run a new experiment.`)};card.append(pick);
