...
function threadCaption(cfg){return `${cfg.threads} threads`}
function updateEstimate(){const cfg=toConfig(mode,values[mode]);if(system.performance_cores)cfg.performance_cores=system.performance_cores;applyDurationCap(cfg);const hours=estimateSeconds(cfg)/3600,cap=hours<=WALL_CAP_HOURS;const span=mode==='galaxy'?`${Math.round(cfg.duration*MYR_PER_TIME).toLocaleString('en-US')} Myr`:`${cfg.duration} yr`;const size=mode==='galaxy'?`${fmt(cfg.n)} particles · ${threadCaption(cfg)}`:`${cfg.perturber_mass>0?10:9} bodies`;const steps=mode==='galaxy'?Math.round(cfg.duration/cfg.dt):null;const maxSpan=mode==='galaxy'?Math.floor(maxDuration(cfg)):maxDuration(cfg);
renderHero(cfg);
const ng=mode==='galaxy'?Number(cfg.n_galaxies||1):1;
const galaxies=mode==='galaxy'?(ng>1?`${fmt(cfg.n)} particles in ${ng} galaxies · ${threadCaption(cfg)}`:`${fmt(cfg.n)} particles · ${threadCaption(cfg)}`):size;
