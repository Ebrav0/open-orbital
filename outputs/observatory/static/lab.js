...
if(lc){const cnt=lc.counts||{};dd(dl,'Gas / stellar / remnant mass',`${fmtNum(lc.gas_mass)} / ${fmtNum(lc.stellar_mass)} / ${fmtNum(lc.remnant_mass)} ×10¹⁰ M☉`);if(lc.cold_gas_mass!=null||lc.hot_gas_mass!=null)dd(dl,'Cold / hot gas mass',`${fmtNum(lc.cold_gas_mass)} / ${fmtNum(lc.hot_gas_mass)} ×10¹⁰ M☉`);if(lc.mean_temperature!=null)dd(dl,'Gas temperature (mass-weighted)',`${fmt(Math.round(lc.mean_temperature))} K${lc.blast_delayed?` · ${fmt(lc.blast_delayed)} parcels in blastwave delay`:''}`);if(lc.mean_metallicity!=null)dd(dl,'Gas metallicity',`${fmtNum(lc.mean_metallicity)} Z☉`);dd(dl,'Star-formation rate',`${fmtNum(lc.sfr*1e4)} ×10⁶ M☉ / Myr`);dd(dl,'Mean stellar age',`${fmt(Math.round(lc.mean_stellar_age||0))} Myr`);dd(dl,'Births / deaths / supernovae',`${fmt(lc.births_cumulative||0)} / ${fmt(lc.deaths_cumulative||0)} / ${fmt(lc.supernovae_cumulative||0)}`);dd(dl,'Population',`cold gas ${fmt(cnt.gas||0)} · hot gas ${fmt(cnt.hot_gas||0)} · proto ${fmt(cnt.protostar||0)} · MS ${fmt(cnt.main_sequence||0)} · giant ${fmt(cnt.giant||0)} · WD ${fmt(cnt.white_dwarf||0)} · NS ${fmt(cnt.neutron_star||0)} · BH ${fmt(cnt.black_hole||0)}${cnt.smbh?' · SMBH 1':''}`);if(lc.mass_return_failed>0)dd(dl,'Mass return kept on remnants (no gas nearby)',`${fmtNum(lc.mass_return_failed)} ×10¹⁰ M☉`)}
