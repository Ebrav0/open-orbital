...
const DRAFT_KEY='orbital-lab-draft-v4',ACTIVE=['running','initializing','pausing'];
...
function markUnused(){if(mode!=='galaxy')return;const G=Number(values.galaxy.n_galaxies||1);const lc=Boolean(values.galaxy.lifecycle_enabled);const ism=Boolean(values.galaxy.ism_enabled);
for(const id in rows){const {out,slider:s,row}=rows[id];if(!row)continue;let unused=false,why='';
if(s.galaxy){unused=G<s.galaxy;why=`not used unless galaxy count ≥ ${s.galaxy}`}
else if(s.group==='Stellar lifecycle'&&s.id!=='lifecycle_enabled'){unused=!lc;why='not used unless stellar lifecycle is on'}
else if(s.group==='Interstellar medium'&&s.id==='ism_enabled'){unused=!lc;why='not used unless stellar lifecycle is on'}
else if(s.group==='Interstellar medium'){unused=!lc||!ism;why=!lc?'not used unless stellar lifecycle is on':'not used unless two-phase ISM is on'}
row.classList.toggle('unused',unused);out.textContent=unused?why:readout(s,values.galaxy[s.id])}}
...
function drawCharts(points){drawSpark('chart-radius',points.map(p=>p.disk_half_radius),'#9fc6b1');drawSpark('chart-sfr',points.map(p=>p.sfr!=null?p.sfr:p.births),'#e0b07a');drawSpark('chart-energy',points.map(p=>p.energy_change),'#8bb4d9');drawSpark('chart-temp',points.map(p=>p.mean_temperature),'#ff8aa0')}
...
if(lc){const cnt=lc.counts||{};dd(dl,'Gas / stellar / remnant mass',`${fmtNum(lc.gas_mass)} / ${fmtNum(lc.stellar_mass)} / ${fmtNum(lc.remnant_mass)} ×10¹⁰ M☉`);if(lc.cold_gas_mass!=null||lc.hot_gas_mass!=null)dd(dl,'Cold / hot gas mass',`${fmtNum(lc.cold_gas_mass)} / ${fmtNum(lc.hot_gas_mass)} ×10¹⁰ M☉`);if(lc.mean_temperature!=null)dd(dl,'Gas temperature (mass-weighted)',`${fmt(Math.round(lc.mean_temperature))} K${lc.blast_delayed?` · ${fmt(lc.blast_delayed)} parcels in blastwave delay`:''}`);if(lc.mean_metallicity!=null)dd(dl,'Gas metallicity',`${fmtNum(lc.mean_metallicity)} Z☉${lc.metal_std!=null?` · spread ${fmtNum(lc.metal_std)}`:''}`);if(lc.mean_electron_fraction!=null)dd(dl,'Ionization / lag',`${fmtNum(lc.mean_electron_fraction)} · |x−x_eq| ${fmtNum(lc.x_lag||0)}`,'Carried electron fraction x_e. Dense gas sits near equilibrium each leapfrog step; diffuse gas can lag.');if((lc.cloud_heat||0)>0||(lc.sn_momentum||0)>0)dd(dl,'Cloud heat / snowplow momentum',`${fmtNum(lc.cloud_heat)} / ${fmtNum(lc.sn_momentum)}`,'Lost intra-cell peculiar kinetic energy, and momentum deposited by the snowplow. Laboratory scales, not resolved remnants.');if((lc.shock_heat||0)>0||(lc.sn_heat||0)>0)dd(dl,'Laboratory heat (shock / SN dump)',`${fmtNum(lc.shock_heat)} / ${fmtNum(lc.sn_heat)} code energy`,'Shock/ram heat is lost bulk kinetic energy on the density grid. SN dump is the superparticle superbubble coupling, not 10⁵¹ erg per resolved star.');dd(dl,'Star-formation rate',`${fmtNum(lc.sfr*1e4)} ×10⁶ M☉ / Myr`);dd(dl,'Mean stellar age',`${fmt(Math.round(lc.mean_stellar_age||0))} Myr`);dd(dl,'Births / deaths / supernovae',`${fmt(lc.births_cumulative||0)} / ${fmt(lc.deaths_cumulative||0)} / ${fmt(lc.supernovae_cumulative||0)}`);dd(dl,'Population',`cold gas ${fmt(cnt.gas||0)} · hot gas ${fmt(cnt.hot_gas||0)} · proto ${fmt(cnt.protostar||0)} · MS ${fmt(cnt.main_sequence||0)} · giant ${fmt(cnt.giant||0)} · WD ${fmt(cnt.white_dwarf||0)} · NS ${fmt(cnt.neutron_star||0)} · BH ${fmt(cnt.black_hole||0)}${cnt.smbh?' · SMBH 1':''}`);if(lc.mass_return_failed>0)dd(dl,'Mass return kept on remnants (no gas nearby)',`${fmtNum(lc.mass_return_failed)} ×10¹⁰ M☉`)}
...
loadDraft();$('notes').value='From Cursor, Grok 4.7: snowplow, cloud drag, metals, local FUV (revision 6)';setMode(mode);schedule();poll();
