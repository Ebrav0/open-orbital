...
export const GALAXY_SLIDERS=[
  ...
  {id:'ism_enabled',group:'Interstellar medium',label:'Two-phase ISM',kind:'bool',default:true,hint:'Grid pressure, ram drag, snowplow, cloud drag, CIE-like cooling and blastwave-delayed SN heat. Not SPH. Needs the stellar lifecycle.'},
  {id:'metallicity',group:'Interstellar medium',label:'Gas metallicity',min:0,max:3,step:.05,default:1,unit:'Z/Z☉',hint:'Scales metal-line cooling and the initial metal mass fraction (Z☉ = 0.02).'},
  {id:'cooling_speed',group:'Interstellar medium',label:'Cooling speed',min:0,max:4,step:.05,default:1,unit:'× CIE approach',hint:'1 ≈ snap toward the Field warm/hot attractor each gravity step. 0 freezes internal energy. Laboratory.'},
  {id:'sn_feedback',group:'Interstellar medium',label:'Supernova heat coupling',min:0,max:1,step:.01,default:.15,unit:'of superbubble dump',hint:'Fraction of a 200 km/s laboratory superbubble dumped into neighbouring gas. Superparticle slots are 10⁴–10⁶ M☉, so this is not 10⁵¹ erg per resolved star.'},
  {id:'ram_pressure',group:'Interstellar medium',label:'Ram-pressure strength',min:0,max:3,step:.05,default:1,unit:'× Gunn–Gott drag',hint:'Momentum-conserving gas–gas drag on the density grid. Visible in encounters as stripped/shocked tails.'},
  {id:'n_sf',group:'Interstellar medium',label:'Star-formation density',min:.01,max:5,step:.01,default:.1,unit:'cm⁻³',hint:'Cold gas below 3.5×10⁴ K must also sit above this n_H and not be strongly expanding. Superparticle-scaled, not a resolved GMC threshold.'},
  {id:'sn_momentum',group:'Interstellar medium',label:'Snowplow momentum',min:0,max:2,step:.05,default:.4,unit:'× 200 km/s ejecta',hint:'Momentum-conserving kick shared by neighbouring gas and the remnant. 0 is heat only. Capped near 144 km/s per event. Laboratory, not a resolved blast.'},
  {id:'cloud_dissipation',group:'Interstellar medium',label:'Cloud collision drag',min:0,max:4,step:.05,default:1,unit:'× cell crossing',hint:'Dissipates peculiar motion inside a density cell. The cell bulk velocity is unchanged. Lost kinetic energy heats the gas.'},
  {id:'metal_diffusion',group:'Interstellar medium',label:'Metal mixing',min:0,max:3,step:.05,default:.6,unit:'× face exchange',hint:'Conservative metal exchange across cell faces. Total metal mass is kept. Not a resolved yield model.'},
  {id:'fuv_heating',group:'Interstellar medium',label:'Local FUV from young stars',min:0,max:4,step:.05,default:1,unit:'× nearby massive stars',hint:'Stars above 8 M☉ and younger than 30 Myr brighten their cell and the six neighbours. Dense gas is shielded. With no young stars this knob does nothing.'},
  {id:'noneq_ionization',group:'Interstellar medium',label:'Non-equilibrium ionization',kind:'bool',default:true,hint:'Carry electron fraction x_e and relax it toward coronal plus photo equilibrium. Off snaps to equilibrium every step. Dense gas is already near equilibrium at this leapfrog step.'},
...
export function estimateSeconds(cfg){if(cfg.mode==='planets'){const bodies=cfg.perturber_mass>0?10:9;return .25*cfg.duration/12*(bodies/9)**2}const n=cfg.n,steps=cfg.duration/cfg.dt;const lc=cfg.lifecycle_enabled?1.15:1;const ism=cfg.lifecycle_enabled&&cfg.ism_enabled!==false?1.18:1;return REFERENCE_SECONDS*(n/1e5)*Math.log(n)/Math.log(1e5)*(steps/500)*(10/cfg.threads)*lc*ism*((cfg.n_galaxies||1)>1?1.05:1)}
...
export function engineRamGb(cfg){return cfg.mode==='planets'?.12:.15+cfg.n*7e-7*(cfg.lifecycle_enabled?(cfg.ism_enabled!==false?1.45:1.2):1)}
