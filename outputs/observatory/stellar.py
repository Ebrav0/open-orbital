...
def new_baryons(rng,n,disk_count,params):
    """Initial lifecycle arrays. First gas_fraction of disk slots are gas; the rest are an alive stellar population."""
    types=np.full(n,HALO,np.uint8);m_star=np.zeros(n);age=np.zeros(n);birth=np.zeros(n)
    ng=int(round(disk_count*float(params['gas_fraction'])));types[:ng]=GAS
    ns=disk_count-ng
    if ns>0:
        ...
        types[ng:disk_count]=np.where(ages>=.9*tms,GIANT,MS);m_star[ng:disk_count]=ms;age[ng:disk_count]=ages;birth[ng:disk_count]=-ages/MYR_PER_TIME
    return dict(type=types,age=age,m_star=m_star,birth_time=birth,disk_count=int(disk_count),debt=0.,born_mass=0.,born_window_myr=0.,supernovae=0,deaths=0,births=0,failed_return=0.)

def save_baryons(path,b):
    np.savez(path,type=b['type'],age=b['age'],m_star=b['m_star'],birth_time=b['birth_time'],scalars=np.array([b['disk_count'],b['debt'],b['born_mass'],b['born_window_myr'],b['supernovae'],b['deaths'],b['births'],b['failed_return']],dtype=np.float64))

def load_baryons(path):
    z=np.load(path);s=z['scalars']
    return dict(type=z['type'].astype(np.uint8),age=z['age'],m_star=z['m_star'],birth_time=z['birth_time'],disk_count=int(s[0]),debt=float(s[1]),born_mass=float(s[2]),born_window_myr=float(s[3]),supernovae=int(s[4]),deaths=int(s[5]),births=int(s[6]),failed_return=float(s[7]))
...
def step(sim,b,params,dt_model,seed):
    ...
    nd=b['disk_count'];types=b['type'];age=b['age'];m_star=b['m_star']
    ...
    pos=q[:nd,:3];dm=np.zeros(nd);dv=None;changed=False
    disk_types=types[:nd]
    gas_idx=np.flatnonzero(disk_types==GAS)
    gas_keys=None;gas_order=None
    if len(gas_idx):
        keys=_cells(pos[gas_idx]);gas_order=np.argsort(keys,kind='stable');gas_keys=keys[gas_order]
    # ---- aging ----
    alive=(disk_types>=PROTO)&(disk_types<=GIANT)
    age[:nd][alive]+=dt_myr
    tms=lifetime_myr(np.where(alive,m_star[:nd],1.))
    # protostar -> MS
    grow=np.flatnonzero((disk_types==PROTO))
    ...
    disk_types[done]=MS
    # MS -> giant
    ms_idx=np.flatnonzero(disk_types==MS);to_giant=ms_idx[age[ms_idx]>=.9*tms[ms_idx]];disk_types[to_giant]=GIANT
    # giant -> remnant
    giants=np.flatnonzero(disk_types==GIANT);dying=giants[age[giants]>=tms[giants]]
    ...
    if len(dying):
        ...
        disk_types[dying]=rtype;b['deaths']+=len(dying);b['supernovae']+=int(np.sum(m_star[dying]>=8));changed=True
        if kicks:
    ...
    if gas_mass>0:
        ...
        if k>0:
            # density per gas particle = occupancy of its grid cell (O(N), no pair loop)
            ...
            disk_types[pick]=np.where(ms>=2.,PROTO,MS)
    b['born_window_myr']+=dt_myr
    # ---- write back ----
    if changed and np.any(dm):
        m[:nd]+=dm;m[:nd]=np.maximum(m[:nd],0.)
...
def summary(sim,b,m=None):
    ...
    t=b['type'];nd=b['disk_count'];counts={TYPE_NAMES[i]:int(np.sum(t==i)) for i in range(9)}
    ...
    out=dict(gas_mass=float(np.sum(m[t==GAS])),stellar_mass=float(np.sum(m[alive])),remnant_mass=float(np.sum(m[(t>=WD)&(t<=BH)])),counts=counts,
      ...
      supernovae_cumulative=int(b['supernovae']),deaths_cumulative=int(b['deaths']),births_cumulative=int(b['births']),mass_return_failed=float(b['failed_return']),baryon_mass=float(np.sum(m[:nd])))
