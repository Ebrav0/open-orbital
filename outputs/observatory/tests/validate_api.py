"""Isolated end-to-end API test for model revision 3 on port 8767 with a temp data dir.
Covers: knob validation, over-budget rejection, pause, restart + checkpoint recovery, 24-byte frames,
summary view, /lab page, log tail, preview, and DELETE rules. Writes api_validation_r3.json (never api_validation.json)."""
...
with tempfile.TemporaryDirectory(prefix='orbital-api-') as td:
    ...
    try:
        proc=start()
        # Static pages: /lab must be HTML with no Three.js reference; / is the viewer.
        ...
        # Validation: bad knob, over-budget duration, bad choice.
        ...
        # Galaxy job with every knob supplied; lifecycle on; SMBH on.
        cfg=dict(mode='galaxy',n=10000,threads=4,duration=4,seed=99,disk_mass=1.2,halo_mass=18,disk_fraction=.3,disk_scale=1.1,disk_thickness=.08,halo_scale=4,warmth=.9,smbh_mass=.005,lifecycle_enabled=True,gas_fraction=.4,t_sf=.6,lifecycle_speed=40,sf_density_bias=.7,imf_mmin=.08,imf_mmax=100,grow_rate=.2,sn_kick_kms=20,theta=.4,softening=.06,dt=.02,notes='api test')
        j=api('/api/jobs',cfg);jid=j['id'];url=f'/api/jobs/{jid}';assert j['config']['smbh_mass']==.005 and j['config']['notes']=='api test' and j['config']['estimated_seconds']>0
        until(lambda:(q:=api(url))['status']['frames']>2 and q)
        assert expect(400,lambda:api('/api/jobs',dict(mode='planets')))
        expect(400,lambda:api(url,method='DELETE'))          # running job cannot be deleted
        ...
        meta=paused['meta'];assert meta['bytes_per_particle']==24 and meta['frame_layout']=='xyzsmt' and meta['model_revision']==3 and meta['smbh_count']==1 and meta['lifecycle_enabled']
        first=api(url+'/frames?start=0');assert len(first)==10000*24
        import numpy as np;f0=np.frombuffer(first,'<f4').reshape(-1,6);assert set(np.unique(f0[:,5]).tolist())<= {0.,1.,2.,3.,4.,5.,6.,7.,8.} and abs(f0[:,4].sum()-(1.2+18+.005))<1e-3
        # Summary view is compact and omits bulky arrays.
        summary=api('/api/jobs?view=summary');me=[x for x in summary if x['id']==jid][0];assert 'times' not in me['meta'] and 'initial_diagnostics' not in me['meta'] and 'protected' in me and me['protected'] is False
        result['summary_bytes_per_job']=len(json.dumps(me));assert result['summary_bytes_per_job']<8000
        assert 'times' in api(url)['meta']
        # Checkpoint pair includes baryon arrays.
        ...
        # Restart the server: job becomes interrupted; resume completes from the checkpoint with identical first frame.
        ...
        lc=end['status']['diagnostics']['lifecycle'];assert lc['births_cumulative']>0 and abs(lc['baryon_mass']-1.2)<1e-6
        ...
        # Log tail and preview.
        log=api(url+'/log?tail=10');assert isinstance(log['lines'],list)
        pv=api('/api/preview',dict(mode='galaxy',n=100000,warmth=.6));assert len(pv)==8000*24
        # Planets with scaled masses and a perturber.
        ...
        # DELETE rules: protected id -> 400; unknown id -> 404; finished job -> folder gone, then 404.
