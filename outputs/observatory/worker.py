...
import numpy as np
from physics import galaxy,planets,arrays,diagnostics,set_threads,rebound,GALAXY_DEFAULTS
import stellar
...
def run(folder):
    ...
    try:
        if (folder/'checkpoint.json').exists():
            ...
            status.update(frames=1,phase='running',computed_time=0,diagnostics=initial,events=['Initial conditions created.'],flags={})
        ...
        def checkpoint():
            ...
            # Keep the committed checkpoint pair and its predecessor; never invalidate the pointer.
        ...
        def interrupt_now():
            status['phase']='interrupted';status['chunk_started_at']=None;checkpoint();atomic(folder/'status.json',status)
        def enter_pause():
        ...
        with (folder/'frames.bin').open('ab') as f:
            while index+1<len(meta['times']):
                if stopping[0]:
                    interrupt_now();return
                if requested_action()=='pause':
                    if enter_pause():interrupt_now();return
                    continue
                ...
                if config['mode']=='galaxy':
                    ...
                    if outcome=='stop':interrupt_now();return
